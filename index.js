...
function validarCorreio(texto) {
  return texto.toLowerCase().includes('cep') &&
    texto.match(/\d{5}-?\d{3}/);
}
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    switch (estado[jid]) {
      ...
      case 'pedido':
        ...
        await sock.sendMessage(jid, { text: 'Pedido registrado. Em breve confirmamos.' });
        return;

      case 'rastreio':
        await sock.sendMessage(jid, { text: 'Vamos verificar seu pedido.' });
        estado[jid] = 'menu';
        return;

      case 'nota':
        await sock.sendMessage(jid, { text: 'Nota em processamento.' });
        estado[jid] = 'menu';
        return;
    }
